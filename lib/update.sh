...
download_file() {
	...
	local path="$2"
	local http_code
	http_code=$(curl -sSL --max-time 60 -o "$path" -w "%{http_code}" "$url")
	# 404: 远端已删除, 同步删本地副本
...
update_script() {
	# 写权限与 root 提升校验
	local perm_res
	check_write_permission "update"
	perm_res=$?
	if [ $perm_res -eq 2 ]; then
		return 0
	elif [ $perm_res -ne 0 ]; then
		break_end
		return 1
	fi
	...
	mkdir -p "$backup_dir"
	cp -r "${LINUXBOX_LIB_DIR}/lib" "$backup_dir/"
	cp -r "${LINUXBOX_LIB_DIR}/modules" "$backup_dir/"
	cp "${LINUXBOX_LIB_DIR}/${SCRIPT_FILE}" "$backup_dir/"
	...
	for entry in "${entries[@]}"; do
		...
		if [ -z "$subdir" ]; then
			...
			if download_file "${url_proxy}raw.githubusercontent.com/${SCRIPT_REPO_OWNER}/${SCRIPT_REPO_NAME}/${SCRIPT_BRANCH}/${path}" "$tmp"; then
				if chmod +x "$tmp" && cp -f "$tmp" "${LINUXBOX_LIB_DIR}/${path}"; then
					entry_ok=1
				else
					FAILED_FILES+=("${path}")
				fi
	...
	if [ $entry_ok -eq 1 ] && [ $total_failed -eq 0 ]; then
		...
		if [ $entry_ok -eq 0 ]; then
			...
			cp -f "${backup_dir}/${SCRIPT_FILE}" "${LINUXBOX_LIB_DIR}/${SCRIPT_FILE}"
		fi
...
rollback_version() {
	...
	# 写权限与 root 提升校验
	local perm_res
	check_write_permission "rollback"
	perm_res=$?
	if [ $perm_res -eq 2 ]; then
		return 0
	elif [ $perm_res -ne 0 ]; then
	...
	if [[ "$confirm" =~ ^[Yy]$ ]]; then
		...
		cp -r "${LINUXBOX_LIB_DIR}/lib" "$current_backup/"
		cp -r "${LINUXBOX_LIB_DIR}/modules" "$current_backup/"
		cp "${LINUXBOX_LIB_DIR}/${SCRIPT_FILE}" "$current_backup/"

		[ -n "${LINUXBOX_LIB_DIR:-}" ] && rm -rf "${LINUXBOX_LIB_DIR}/lib" "${LINUXBOX_LIB_DIR}/modules"
		if cp -r "${backup_path}/lib" "${LINUXBOX_LIB_DIR}/" && \
		   cp -r "${backup_path}/modules" "${LINUXBOX_LIB_DIR}/" && \
		   cp "${backup_path}/${SCRIPT_FILE}" "${LINUXBOX_LIB_DIR}/"; then
			echo -e "${green}✓ 回滚完成，请重新运行脚本${white}"
			break_end
			exit 0
		else
			echo -e "${red}错误: 回滚过程中文件复制失败！${white}"
			break_end
			return 1
		fi
	fi
